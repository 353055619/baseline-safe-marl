...
def run_episode(policy, env, max_steps: int, seed: int = None):
    ...
    while not done and step_count < max_steps:
        action_dict = {agent: policy.get_actions(obs_dict[agent], deterministic=True) for agent in env.agents}
...
def main():
    ...
    parser.add_argument("--algo", type=str, required=True, choices=["MAPPO", "MAPPO-L", "HAPPO", "MACPO", "MATD3"])
    ...
    for run_idx in range(args.runs):
        for ep in range(1, args.episodes + 1):
            ...
            stats = run_episode(policy, env, max_steps=args.max_steps, seed=seed)
            env.close()
            r = round(stats["reward"], 4)
            c = round(stats["cost"], 4)
            rows.append({"algo": args.algo, "episode": ep, "reward": r, "cost": c, "steps": stats["steps"], "done": stats["done"]})
            print(f"  [{args.algo}] run={run_idx+1}/{args.runs} ep={ep}/{args.episodes} reward={r:.3f} cost={c:.3f}")
